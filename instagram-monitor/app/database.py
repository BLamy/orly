...
Verwaltet drei Tabellen:
  ...
  posts      – alle bekannten Beiträge (Shortcodes) pro Konto; verhindert
               doppelte Benachrichtigungen und doppelte Downloads
...
CREATE TABLE IF NOT EXISTS posts (
    shortcode     TEXT PRIMARY KEY,               -- eindeutige Instagram-Kurz-ID des Beitrags
    username      TEXT NOT NULL,                  -- zu welchem Konto der Beitrag gehört
    post_type     TEXT NOT NULL DEFAULT '',       -- z. B. "Reel", "Video", "Album", "Bild"
...
class Database:
    ...
    # Beiträge
    # ------------------------------------------------------------------
    def is_post_known(self, shortcode: str) -> bool:
        """Wurde dieser Beitrag schon einmal registriert?"""
        rows = self._query("SELECT 1 FROM posts WHERE shortcode = ?", (shortcode,))
        return bool(rows)
    ...
    def add_post(
        ...
        self._execute(
            ...
            VALUES (?, ?, ?, ?, ?)
            """,
            (shortcode, username, post_type, posted_at, _now()),
        )

    def mark_downloaded(self, shortcode: str) -> None:
        """Markiert einen Beitrag als erfolgreich heruntergeladen."""
        self._execute(
            "UPDATE posts SET downloaded = 1, downloaded_at = ? WHERE shortcode = ?",
            (_now(), shortcode),
        )
