...
{% viz scene="books/one-project-many-environments/chapter-1" section="chapter-1-single-slot" cue="0" from="0.000" to="24.950" title="One target URL slot means every other deployment bounces off or overwrites it." %}
{% endviz %}
...
{% viz scene="books/one-project-many-environments/chapter-1" section="chapter-1-override" cue="3" from="24.950" to="51.537" title="An override URL carries no logins, no instructions, and no environment identity." %}
{% endviz %}
...
{% viz scene="books/one-project-many-environments/chapter-1" section="chapter-1-statement" cue="6" from="51.537" to="68.081" title="The problem statement: single-slot configuration and anonymous run history." %}
{% endviz %}
...
`projects.target_url`, `projects.test_instructions`, and `projects.global_variables` take on a clear role: the project-wide defaults. Beside them, a `project_environments` row carries a name, a kind, a trigger, and three *optional* fields — `target_url`, `test_instructions`, `environment_variables` — that shadow those defaults. The foreign key makes it one project, many environments: production, staging, and a preview for every pull request, all standing on the same defaults.

{% viz scene="books/one-project-many-environments/chapter-2" section="chapter-2-overlay" cue="0" from="0.000" to="32.124" title="Project fields as defaults; environments as overlays with optional shadow fields." %}
{% endviz %}
...
{% viz scene="books/one-project-many-environments/chapter-2" section="chapter-2-fallback" cue="3" from="32.124" to="62.286" title="environment.field ?? project.field — override where defined, inherit where not." %}
{% endviz %}
...
{% viz scene="books/one-project-many-environments/chapter-2" section="chapter-2-runs" cue="6" from="62.286" to="73.025" title="One nullable column: null means project defaults, set means a named world." %}
{% endviz %}
...
{% viz scene="books/one-project-many-environments/chapter-2" section="chapter-2-exclusions" cue="7" from="73.025" to="90.231" title="An empty exclusions table changes nothing; one row scopes a journey." %}
{% endviz %}
...
{% viz scene="books/one-project-many-environments/chapter-3" section="chapter-3-production" cue="0" from="0.000" to="20.851" title="Production runs nightly on weekdays, on a schedule the environment owns." %}
{% endviz %}
...
{% viz scene="books/one-project-many-environments/chapter-3" section="chapter-3-staging" cue="3" from="20.851" to="35.585" title="Staging fires on deployment-pipeline completion, not on the merge." %}
{% endviz %}
...
{% viz scene="books/one-project-many-environments/chapter-3" section="chapter-3-preview" cue="5" from="35.585" to="54.810" title="Preview runs gate on ready-for-review AND a live preview deployment." %}
{% endviz %}
