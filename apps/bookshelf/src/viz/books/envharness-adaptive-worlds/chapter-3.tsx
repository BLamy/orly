...
export function buildScene() {
  ...
  tl.hold(62.4, 1.0);
  return { tl, cam, reelA, reelB, linkU, aProgress, aVerify, handoff, contextU, bProgress, bVerify, resultU, evidenceU, close };
}
  ...
  return <Camera {...s.get(scene.cam)}>
    <g opacity={quiet}>
      ...
      <g opacity={s.get(scene.evidenceU)}>
        <rect x="420" y="205" width="440" height="96" rx="22" fill="#111827" stroke={colors.WARM} strokeWidth="3" />
        <text x="640" y="243" textAnchor="middle" fill={colors.WARM} fontSize="17" fontWeight="700">Stage + Contract + Chain skills</text>
        <text x="640" y="274" textAnchor="middle" fill={colors.TEXT} fontSize="18">SR 54.30% · average steps 43.12</text>
      </g>
      ...
      <g opacity={Math.min(s.get(scene.reelA), s.get(scene.reelB))}>
        ...
        <text x="950" y="470" textAnchor="middle" fill={colors.MUTED} fontSize="13">lazy reset at handoff</text>
      </g>
    </g>
