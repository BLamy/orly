...
const base=process.env.ORLY_STORYBOOK_URL||'http://127.0.0.1:6007';
...
const seek=page.getByRole('slider',{name:'Seek',exact:true});const step=Number(await seek.getAttribute('step'));const t=Math.min(c.at+c.dur*.6,ex.duration-.1);await seek.fill(String(Number((Math.round(t/step)*step).toFixed(3))));await page.waitForTimeout(100);await page.locator('.viz-player-stage').screenshot({path:path.join(dir,`draft-chapter-${chapter}-cue-${i+1}.png`)});
}
